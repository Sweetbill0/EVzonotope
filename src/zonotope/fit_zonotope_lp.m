...
%   normSpec : struct with fields t_min, E_min, scale_t, scale_E and
%              optionally offset (2-by-1) defining the origin shift used
%              during normalisation (optional).
%
%   Output
%   ------
...
arguments
    ...
    normSpec struct = struct('t_min', 0, 'E_min', 0, 'scale_t', 1, 'scale_E', 1, 'offset', [0; 0])
end
...
% Normalisation -------------------------------------------------------------
S = diag([normSpec.scale_t; normSpec.scale_E]);
S_inv = diag(1 ./ diag(S));
if isfield(normSpec, 'offset')
    offset = normSpec.offset;
else
